
<?php
$phan_logo = "data:image/svg+xml;base64,";
$sol_logo = "data:image/svg+xml;base64,";
?>

<div class="main_body">
    <div class="content__pair">
        <div class="content__pair--wide">
            <div>
                <?php echo __('In order to finalise your order you must finish the payment process. Please follow these instructions:', 'fyfy_usdc');?>
            </div>
            <br>
            <div>
                <?php echo _e('1. Open Fyfy Pay application on your mobile device
                <br>
                2. Click “Pay” on the home screen
                <br>
                3. Scan the QR-code
                <br>
                4. Swipe to confirm the payment
                <br>
                5. Wait for payment verification', 'fyfy_usdc');?>
            </div>
            <br>
            <div>
                <?php echo __('After payment is complete your order will be handled and sent.', 'fyfy_usdc');?>
            </div>
        </div>
        <div class="content__pair--narr">
            <h4>
                <?php echo __('Scan this code to pay', 'fyfy_usdc');?>
            </h4>
            <div class="fyfy_qr">
                <div id="qrcode" alt="QR code is disabled!">
            </div>
        </div>
    </div>
    <div class="content__pair">
        <div class="content__pair--header">
            <div><?php echo __('or pay with browser wallet', 'fyfy_usdc');?></div>
        </div>

        <div class="content__pair--content">
            <div class="content__pair--half">
                <button class="pay_broswer_button phan">
                    <i class="wc-wallet-button-icon">
                        <img src="<?php echo esc_attr($phan_logo) ?>">
                    </i>
                    <?php echo __('Phantom', 'fyfy_usdc'); ?>
                </button>
            </div>
            <div class="content__pair--half">
                <button class="pay_broswer_button sol">
                    <i class="wc-wallet-button-icon">
                        <img src="<?php echo esc_attr($sol_logo) ?>">
                    </i>
                    <?php echo __('Solflare', 'fyfy_usdc'); ?>
                </button>
            </div>
        </div>
    </div>
    <div class="content__pair">
        <div class="content__pair--header">
            <h4><?php echo __('Waiting for payment', 'fyfy_usdc');?></h4>
        </div>
        <div class="content__pair--content">
            <div class="content__pair--subheader">
                <?php echo __('Receivers wallet address', 'fyfy_usdc'); ?>
            </div>
            <div class="content__pair--subcontent">
                <strong>
                        <span class="woocommerce-Price-amount amount">
                            <?php echo '<span class="all-copy ">' . esc_html($store_address) . ' <span class="storewalletaddress clipboard far fa-copy" title="Copy to clipboard" data-value="' . esc_html($store_address) . ' "></span></span>' ?>
                        </span>
                </strong>
            </div>

            <div class="content__pair--subheader">
                <?php echo __('Payment amount', 'fyfy_usdc'); ?>
            </div>

            <div class="content__pair--subcontent">
                <?php
                echo '<img style="display:inline;height:18px;width:18px;vertical-align:middle;" src="' . $usdc_logo_url. '" />';
                echo '<strong><span class="all-copy">&nbsp;&nbsp;' . esc_html($order_total_price) . ' </span>'. '<span class="no-copy">' . __('USDC', 'fyfy_usdc') . '</span></strong>';
                ?>
            </div>
            <div class="content__pair--subheader"></div>
            <div class="content__pair--subcontent">
                <div class="processing_image loading" id="fyfyBtn">
                <img src="<?php echo FYFY_PLUGIN_URL.'/assets/images/payment_processing_animation.gif';?>">
                </div>
            </div>
            <div class="content__pair--subheader">
                <?php echo __('[click to see confirmation]', 'fyfy_usdc'); ?>
            </div>
        </div>
    </div>
    <div class="content__pair">
        <div class="content__pair--header">
            <h4><?php echo __('Don\'t have FyFy Pay?', 'fyfy_usdc');?></h4>
            <p><?php echo __('Download the application for mobile device', 'fyfy_usdc');?></p>
        </div>
        <div class="content__pair--content">
            <div class="content__pair--subheader">
                <a class="app_broswer_link apple" href="https://apps.apple.com/us/genre/ios/id36">
                    <img width="150" src="<?php echo FYFY_PLUGIN_URL.'/assets/images/apple-app-store.png';?>">
                </a>
            </div>

            <div class="content__pair--subheader">
                <a class="app_broswer_link google" href="https://play.google.com/store/apps/">
                    <img width="150" src="<?php echo FYFY_PLUGIN_URL.'/assets/images/google-app-store.png';?>">
                </a>
            </div>
        </div>
    </div>
</div>

<div id="fyfymodal" class="fyfy_modal">
    <!-- Modal content -->
    <div class="modal-content">
        <p class="modal-header">Payment verified</p>
        <div class="processing_image">
            <img width="100" src="<?php echo FYFY_PLUGIN_URL.'/assets/images/payment_confirmation_animation.gif';?>">
        </div>
        <p class="modal-text">Your order is now being handled and will be on its way soon! <br> You can now close his window.</p>
        <div class="modal-footer">
            <a href="<?php echo home_url(); ?>">&#60; Back to home page</a>
        </div>
    </div>

</div>

<script>
    var storeWalletAddress = "<?php echo !empty($store_address)? $store_address:''; ?>";
    var contractaddress = "<?php echo !empty($usdc_contract_address)? $usdc_contract_address:''; ?>";
    var amount = <?php echo !empty($order_total_price)? $order_total_price:0; ?>;
    var order_id = <?php echo !empty($order_id)? $order_id:0; ?>;
    var qr_data = "<?php echo !empty($qr_data)? $qr_data:''; ?>";
</script>