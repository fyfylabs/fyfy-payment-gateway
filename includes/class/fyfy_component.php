...
class fyfy_component{
    ...
    public static function load_file( $file, $folder, $args = [] ) {
        $full_path = FYFY_PLUGIN_DIR .'/includes/'.$folder . '/' . $file;
        /**
         ...
         */
        extract($args);
        $full_path = apply_filters( 'fyfy_load_file_full_path', $full_path, $file, $folder );
        if ( $full_path ) {
    ...
    public static function load_class( $class, $file, $folder, $params = null ) {
         ...
         */
        $class = apply_filters( 'fyfy_load_class_name', $class );
